...
public class User{
	...
	public boolean updateCity(int cityId){
		...
		try{
			...
			String query = "update users set city_id=? where email=?";
	...
	public boolean updatePassword(String newPass){
		...
		try{
			...
			pst.setString(1,spe.encryptPassword(newPass));
			pst.setString(2,email);
	...
	public boolean checkPassword(String oldPass){
		...
		try{
			...
			if(rs.next()){
				StrongPasswordEncryptor spe = new StrongPasswordEncryptor();
				if(spe.checkPassword(oldPass,rs.getString(1))){
					flag = true;
				}
			}
	...
	public boolean loginUser(){
		...
		try{
			...
			String q2 = "profile_pic,city_id from users where email=?";
